...
double dtwUCR(double *s, double *q, int r, int ns, int nq,double bsf , double * distances)
{
    
    double * t;       // Stores the circular array for the data
    ...
    int m=-1;
    long long loc = 0, originalIndex = 0;
    double t1,t2;
        ...
        {   lower_upper_lemire(buffer, ep, r, l_buff, u_buff);
            ...
            for(i=0; i<ep; i++)
            {
                ...
                if( i >= m-1 )
                {
                    ...
                    I = i-(m-1);
                    originalIndex = (it)*(EPOCH-m+1) + i-m+1;
                    distances[originalIndex] = INF;
        ...
        int nrhs, const mxArray *prhs[])
{
  ...
  //  cout << ns << " " << nt << " " << threshold << " " << w << endl;
    
    plhs[0] = mxCreateDoubleMatrix( 1, ns-nt+1, mxREAL);
    
    dp = mxGetPr(plhs[0]);
    
    double * distances = (double *)malloc(sizeof(double)*ns);
    
    double ttt = dtwUCR(s,t,w,ns,nt,threshold, distances);
